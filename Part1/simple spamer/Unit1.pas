...
begin
  logmemo.Lines.Add('Программа-клиент для администраторов групп  для отправки сообщений ВК');
  logmemo.Lines.Add('Автор: студент группы П-41 Руленко Никита');
  logmemo.Lines.Add('Начните работу с авторизации: введите логин и пароль для получения токена');
  logmemo.Lines.Add('Выберите группу из списка Ваших групп (где вы админ)');
  logmemo.Lines.Add('По группе будет загружена статистика, и вы сможете отправить сообщение на стену группы');
  logmemo.Lines.Add('Пустое сообщение отправить НЕЛЬЗЯ! К сообщению можно прикрепить ссылку.');
end;
...
begin
  ...
  if MessageMemo.Text = '' then begin
    checkMsg:= False;
    logmemo.Lines.Add('Нельзя отправить пустое сообщение!');
  end
  else checkMsg:= True;
...
//  logmemo.Lines.Add(id);
  ...
  if checkMsg = True then logmemo.Lines.Add('Сообщение опубликовано!');
...
begin
  ...
  //Здесь дёргаем и заполняем статистику
  ...
  try
    ...
    lstStat.Items.Add('Охват подписчиков: '+subsCount);

    mobile:=pars(response, '"mobile_reach"', ',"');
    lstStat.Items.Add('Охват с телефонов: '+mobile);

    country:=pars(response,'"countries":',']');
    country:=pars(country,'"name":"','"}');
    lstStat.Items.Add('Страна группы: '+country);

    city:=pars(response,'"cities":[{','},');
    city:=pars(city,',"name":"','"');
    lstStat.Items.Add('Город группы: '+city);

    views:=pars(response,'"views":',',"');
    //views:=pars(views,'','');
    lstStat.Items.Add('Просмотров за день: '+views);
...
begin
  ...
  if Pos('token', response)<>0 then begin
    ...
    try
      ...
      logmemo.Lines.Add('Хрен а не группы');
    end;
    //
